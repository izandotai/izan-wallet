...
std::optional<std::string> KeydClient::address(uint32_t account, uint8_t preset)
{
    uint8_t frame[6]
        = { uint8_t(Op::Address), uint8_t(account), uint8_t(account >> 8),
              uint8_t(account >> 16), uint8_t(account >> 24), preset };
    ...
    uint64_t id, const SecureBytes& passphrase)
{
    ...
    if (reply->data()[0] != uint8_t(Op::SignedSol)
        || reply->size() != 1 + kSignedSolBodyBytes) {
        m_last_error.assign(reinterpret_cast<const char*>(reply->data()) + 1,
            reply->size() - 1);
        return std::nullopt;
    }
    std::array<uint8_t, 64> sig {};
    std::memcpy(sig.data(), reply->data() + 1, sig.size());
    return sig;
}
