...
std::vector<TxRecord> parse_txlist(std::string_view json, std::string_view self)
{
    ...
    // status "0" covers both real errors and the perfectly fine "No
    // transactions found"; an empty or non-array result is just an
    // empty ledger.
    ...
    for (const glz::json_t& row : result->second.get_array()) {
        ...
        rec.counterparty = rec.incoming ? field(row, "from") : field(row, "to");
        try {
        ...
        rec.time = when.empty() ? 0 : std::strtoull(when.c_str(), nullptr, 10);
    ...
    const chains::ChainSpec& chain, const std::string& address)
{
    ...
    const std::string target = "/api?module=account&action=txlist&address="
        + address + "&sort=desc&page=1&offset=25";
    ...
    return parse_txlist(res.body, address);
}
